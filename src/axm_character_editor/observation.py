...
def _material_rows(asset: Asset) -> list[tuple]:
    ...
    for node in asset.nodes:
        ...
        for primitive in mesh.get("primitives", []):
            ...
            color_ref = primitive.get("attributes", {}).get("COLOR_0")
            vertex_colors = asset.accessor(color_ref) if color_ref is not None else None
            rows.append((color, ref if isinstance(ref, int) else None, vertex_colors))
    return rows
    ...
    *,
    ...
    for prim_index, (points, record, matrow) in enumerate(zip(deformed, records, material_rows)):
        ...
        for k in range(0, len(indices), 3):
            ...
            if matrow[2] is not None:
                triangle_color = tuple(color[channel] * sum(matrow[2][i][channel] for i in (ia, ib, ic)) / 3 for channel in range(3))
            polygons.append(((za + zb + zc) / 3, pa, pb, pc, _hex(triangle_color, shade)))
            kept += 1
